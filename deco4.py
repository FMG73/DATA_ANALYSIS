...
def ejecutar_con_log(mostrar_alerta=True, cerrar_auto=False, delay_cierre=5):
    ...
    def decorador(func):
        ...
        def wrapper(*args, **kwargs):
            # INTENTO DE DETECCION DEL MAIN DE FORMA SEGURA
            caller_frame = None
            for frame_info in inspect.stack():
                try:
                    if "decoradores" not in frame_info.filename:
                        caller_frame = frame_info
                        break
                except AttributeError:
                    continue

            if caller_frame is not None and hasattr(caller_frame, "filename"):
                ...
                nombre_funcion_main = "INTERACTIVO"
