...
class ReporteTableauOutlook:
    ...
    - Reemplazar hashtags genéricos en el HTML (#VARIABLE#)
    - Enviar correo con dashboards + adjuntos + variables dinámicas

    Los dashboards, adjuntos y variables son completamente opcionales.
    """
    ...
    def reemplazar_variables_html(self, variables):
        """
        Reemplaza hashtags en el HTML del correo por valores de un diccionario.

        :param variables: dict con estructura { "NOMBRE": "Valor a reemplazar" }
                          Por ejemplo: { "FECHA": "16/12/2025", "USUARIO": "Fernando" }
        """

        if not self.mail:
            raise RuntimeError("El mail no está preparado. Ejecuta preparar_mail() primero.")
