...
def crear_xml(row, namespace):
    ...
    # ----------- CORREGIDO: ORDEN DE ATRIBUTOS UOM → xmlns -----------
    power = ET.SubElement(engine, "power")
    power.set("UOM", clean(row["power_UOM"]))
    power.set("xmlns", "")
    ...
    # ----------- CORREGIDO: ORDEN DE ATRIBUTOS UOM → xmlns -----------
    co2 = ET.SubElement(performance, "co2Emission")
    co2.set("UOM", clean(row["co2Emission_UOM"]))
    co2.set("xmlns", "")
    ...
    # ----------- CORREGIDO: ORDEN DE ATRIBUTOS UOM → xmlns -----------
    mileage = ET.SubElement(history, "mileage")
    mileage.set("UOM", "km")
    mileage.set("xmlns", "")
...
seen = {}  # ← (opcional corregir duplicados, pero NO lo toco porque pediste no cambiar funcionalidad)
