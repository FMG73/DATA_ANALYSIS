...
def ejecutar_con_log(mostrar_alerta=True, cerrar_auto=False, delay_cierre=5):
    ...
    Parametros:
    mostrar_alerta : bool → si mostrar ventana Tkinter
    cerrar_auto : bool → si se cierra automáticamente
    delay_cierre : int → segundos antes de cerrar automáticamente
    """
    ...
    def decorador(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            # LOG: inicio decorador
            mensaje_decorador = f"INICIO DECORADOR {ruta_decorador}"
            logger.info(f"[bold magenta]{mensaje_decorador}[/bold magenta]")
            guardar_en_csv(nombre_csv, fecha_inicio, "OK", "INFO", mensaje_decorador)

            # LOG: inicio función main
            mensaje_funcion = f"INICIO FUNCION MAIN {nombre_funcion_main} - DESDE {nombre_archivo_main}"
            logger.info(f"[bold blue]{mensaje_funcion}[/bold blue]")
            guardar_en_csv(nombre_csv, fecha_inicio, "OK", "INFO", mensaje_funcion)
            ...
            if ok:
                mensaje_fin = f"FINALIZA FUNCION MAIN {nombre_funcion_main} OK - DURACION {duracion_formato}"
                logger.info(f"[bold green]{mensaje_fin}[/bold green]")
                guardar_en_archivo(log_ok, fecha_fin, 'INFO', mensaje_fin)
                guardar_en_csv(nombre_csv, fecha_fin, 'OK', 'INFO', mensaje_fin, duracion_formato)
            else:
                mensaje_fin = f"FINALIZA FUNCION MAIN {nombre_funcion_main} KO - DURACION {duracion_formato}"
                logger.error(f"[bold red]{mensaje_fin}[/bold red]")
                guardar_en_archivo(log_ko, fecha_fin, 'ERROR', mensaje_fin)
                guardar_en_csv(nombre_csv, fecha_fin, 'KO', 'ERROR', mensaje_fin, duracion_formato)
