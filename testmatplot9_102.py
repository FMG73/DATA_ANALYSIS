...
def draw_card(ax, title, value, style):
    ...
    ax.text(50, 36, title, ha="center", fontsize=13, color=title_color)
    ax.text(50, 20, f"{int(value):,}".replace(",", "."), ha="center",
...
def draw_row(fig, y_start, height, cards, row_key=None):
    ...
    for i, card in enumerate(cards):
        ax = fig.add_axes([i / n, y_start, 1 / n, height])
        draw_card(ax, card["title"], data[card["col"]], card_style)

# =========================
# CREAR FIGURA Y GUARDAR
# =========================
dashboard_file = "dashboard_kpi_premium.png"
...
ax_title.axis("off")
ax_title.text(0.5, 0.5, DASHBOARD_CONFIG["title"]["text"],
              ha="center", va="center",
...
# FILAS
draw_row(fig, y_start=DASHBOARD_CONFIG["rows"]["row_3_height"],
         height=DASHBOARD_CONFIG["rows"]["row_2_height"],
         cards=ROW_2_CARDS, row_key="row_2")

draw_row(fig, y_start=0,
         height=DASHBOARD_CONFIG["rows"]["row_3_height"],
         cards=ROW_3_CARDS, row_key="row_3")

plt.savefig(dashboard_file, dpi=200, bbox_inches="tight")
plt.close(fig)

# =========================
# INSERTAR EN OUTLOOK
# =========================
ruta_msg = r"C:\ruta_mensaje\mensaje.msg"
imagen_dashboard = os.path.abspath(dashboard_file)

outlook = win32.Dispatch("Outlook.Application")
msg = outlook.CreateItemFromTemplate(ruta_msg)

cid_dashboard = "dashboard1"
html_body = msg.HTMLBody
html_body = html_body.replace("#DASHBOARD#", f'<img src="cid:{cid_dashboard}">')
msg.HTMLBody = html_body

attachment = msg.Attachments.Add(imagen_dashboard)
attachment.PropertyAccessor.SetProperty("http://schemas.microsoft.com/mapi/proptag/0x3712001F", cid_dashboard)

msg.Save()
msg.Display()  # abre el mensaje para revisar antes de enviar
# msg.Send()  # descomenta si quieres enviar automáticamente