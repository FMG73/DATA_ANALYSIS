...
def generar_xml_desde_excel(ruta_entrada: str, ruta_salida: str) -> None:
    ...
    - Detecta todos los errores de filas y columnas obligatorias
    - Vacíos y coefficient no numérico → rojo
    - Duplicados conflictivos → amarillo
    - Solo genera XML si no hay errores
    - XML indentado y legible
    """
    errores = []
    ...
    for index, row in df.iterrows():
        ...
        if not brand:
            errores.append(("FILA {}: BRAND VACÍO".format(fila).upper(), "red"))
        if not model:
            errores.append(("FILA {}: MODEL VACÍO".format(fila).upper(), "red"))
        if coefficient is None or (isinstance(coefficient, float) and pd.isna(coefficient)) or str(coefficient).strip() == "":
            errores.append(("FILA {}: COEFFICIENT VACÍO".format(fila).upper(), "red"))
        else:
            try:
                float(coefficient)
            except (ValueError, TypeError):
                errores.append(("FILA {}: COEFFICIENT NO NUMÉRICO".format(fila).upper(), "red"))

    # 3️⃣ Duplicados conflictivos
    ...
    # 4️⃣ Imprimir todos los errores y salir si existen
    if errores:
    ...
    mensaje_imprimir(
        "RESUMEN FINAL: FILAS PROCESADAS: {} | DUPLICADOS ELIMINADOS: {}".format(
            total_filas, duplicados_eliminados
        ).upper(),
        "blue"
    )