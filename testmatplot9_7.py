...
def draw_card(ax, title, value, style):
    """Dibuja una tarjeta con fondo degradado y sombra"""
    ax.set_xlim(0, 100)
    ax.set_ylim(0, 60)
    ax.axis("off")
    ...
    grad_rgb = np.linspace(
        np.array(plt.colors.to_rgb(color_start)),
        np.array(plt.colors.to_rgb(color_end)),
        256
    )
    grad_array = np.zeros((256,256,3))
    for i in range(3):
        grad_array[:,:,i] = grad_rgb[:,i][:,None]
    ax.imshow(grad_array, extent=[3, 93, 6, 58], aspect='auto', zorder=1)
