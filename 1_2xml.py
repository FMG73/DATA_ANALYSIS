...
def generar_xml_desde_excel(ruta_entrada: str, ruta_salida: str) -> None:
    ...
    # 3️⃣ Validar cada fila y columna
    for index, row in df.iterrows():
        fila = index + 2  # Para que coincida con Excel (cabecera incluida)
        ...
        # Verificar coefficient
        coefficient = row.get("coefficient")
        if coefficient is None or str(coefficient).strip() == "":
    ...
    # 6️⃣ Eliminar duplicados seguros (misma brand+model y mismo coefficient)
    df_antes = len(df)
